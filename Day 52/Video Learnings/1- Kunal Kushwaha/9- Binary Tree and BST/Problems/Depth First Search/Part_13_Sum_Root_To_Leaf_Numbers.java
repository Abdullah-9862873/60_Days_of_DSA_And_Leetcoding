...
public class Part_13_Sum_Root_To_Leaf_Numbers {
    ...
    public static void main(String[] args) {
        ...
        int sum = calcSumRootToLeaf(root);
        System.out.println("Sum Root to leaf is: " + sum);
    }
    public static int calcSumRootToLeaf(Node root){
        int sum = helper(root, "");
        return sum;
    }
    public static int helper(Node root, String str){
        if(root == null){
            return 0;
        }

        str = str + String.valueOf(root.value);
        int left = helper(root.left, str);
        int right = helper(root.right, str);

        int calcVal = left + right;
        if(root.left == null && root.right == null){
            calcVal = Integer.parseInt(str);
        }
        return calcVal;
    }
